...
public class CreateCustomPlane : ScriptableWizard
{
	...
	[MenuItem("EditorScript/CreateObject/CustomPlane")]
	static void CreateWizard()
	{
		...
		ScriptableWizard.DisplayWizard("CreateCustomPlane", typeof(CreateCustomPlane));
    }
		...
		if(m == null)
		{
			...
			for(int y=0; y<vCount2; y++)
			{
				for(int x=0; x<hCount2; x++)
				{
					...
					{
						vertices[index] = new Vector3((x * scaleX) - (width / 2.0f) - anchorOffset.x,
						                              0.0f,
						                              (y * scaleY) - (length / 2.0f) - anchorOffset.y);
					}
					else
					{
						vertices[index] = new Vector3((x * scaleX) - (width / 2.0f) - anchorOffset.x,
